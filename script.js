...
function alignLinks() {
	...
	for (i = 0; i<anchors.length; i++) {
		anchors[i].onmouseover = function() {mouseOverAnchor(this)};
		anchors[i].onmouseout = function() {mouseOutOfAnchor(this)};
		alignLink(i);
...
function alignLink(i) {
	...
	if (left)
		anchor.style.marginLeft = -1 * (radius + containerWidth);
	else
		anchor.style.marginLeft = radius;
	anchor.style.marginLeft += "px";
	if (left)
		anchor.style.transformOrigin = "100% 50%";
	else
		anchor.style.transformOrigin = "0% 50%";
	if (!left)
		rotate *= -1;
	if (!left)
